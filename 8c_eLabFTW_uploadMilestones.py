...
if __name__ == "__main__":
	uploaded_file = "../ressourcen/eLabFTW_milestones_uploaded.tsv"
	projects_uploaded = {}
	...
	with open(projects_file) as f:
		...
		for line in f:
			...
			json_name = "../ressourcen/milestone_jsons/" + split_line[2]
			...
			if comments and not (type(comments)==list):
				comments = eval(comments)
			for comment in comments:
				add_comment_to_experiment(item_id, remove_stuff(comment["comment"]))
			...
			with open("LabguruID_eLabFTWID_all.tsv", "a") as f:
				f.write("milestones\t" + split_line[0] + "\t" + str(item_id) + "\n")
