...
download_path="../ressourcen/experiment_attachments_2/"
...
def download_elements_experiment_procedures(element_id):
	if element_id == 1134711:
		return "Check Labguru what belongs here, something went wrong with downloading", True, [], []
	element_r = requests.get(f'{BASE_URL}/elements/{element_id}', params=standard_params, headers=standard_headers)
	if element_r.status_code == 500:
		print("Error code 500, skipping this")
		return "", False, [],[]
	if not element_r.status_code == 200:
		raise ValueError("function download_elements_experiment_procedures: Error for element " + str(element_id) + ", http status code: " + str(element_r.status_code))
		return 1
	...
	if element_type == "text":
		...
		if type(element_json["data"]) == str:
			try:
				string = json.loads(element_json["data"].replace('\\','')) # part to element_json
			except:
				print("problem in element " + str(element_id))
				problem_occured = True
		elif type(element_json["data"]) == dict:
			string = element_json["data"]
		else:
			raise TypeError(element_json)
		if problem_occured:
			write_text += " Samples could not be linked for some reason, check Labguru if not obvious from surrouding text which samples (plasmid, strains, reagents, ...) were used<br>"
		else:
...
def download_attachment(attachment_info):
	...
	intermit_filename = attach_filename
	filename = download_path + intermit_filename
	while filename in list_of_all_attachments:
		print("Attention ! filename " + filename + " has filename which already occurred! appending 'anoth_' to beginning")
		intermit_filename = "anoth_" + intermit_filename
		filename = download_path + intermit_filename
	...
	if attachment_r.status_code == 200:
		...
		return 0, filename
	elif attachment_r.status_code == 403:
		return 0, ""
	else:
...
def download_experiment(project_number):
	r = requests.get(f"{BASE_URL}/experiments/{project_number}", params=standard_params, headers=standard_headers)
	if not r.status_code == 200:
		raise ValueError("function download_experiment: Error for project " + project_number + ", http status code: " + str(r.status_code))
		return 1
		
	project_info = json.loads(r.text)

	labguru_id = project_info["id"]
	uuid = project_info["uuid"] # important for links
	links = project_info["links"] # important for links
	title = "Experiment: " + project_info["title"]
	project_belongs_to = project_info["project_id"] # link this
	
	start_date = project_info["start_date"] # null if not present
	created_at = project_info["created_at"] # null if not present
	updated_at = project_info["updated_at"]
	
	milestone = project_info["milestone"]["id"] # link this
	protocol = project_info["project"]["id"] # link this
	...
	signed = project_info["signed"]
	if signed:
		signed_at = project_info["signed_at"]
		signed_by = project_info["signed_by"]
	witnessed = project_info["witnessed"]
	if witnessed:
		witnessed_at = project_info["witnessed_at"]
		witnessed_by = project_info["witnessed_by"]
	...
	eLabFTW_prel_links.append("/biocoll/milestones/" + str(milestone))
	
	alert_to_check = False
	for exp_proce in experiment_procedures:
		exp_proc = exp_proce["experiment_procedure"]
	...
	eLabFTW_metadata = {"elabftw": {"extra_fields_groups": [{"id": 1, "name": "General info"}, {"id": 2, "name": "Labguru Info"}]}, "extra_fields": {"Owner": {"type":"text", "value":owner, "group_id":1}, "Labguru_ID": {"type":"text", "value":labguru_id, "group_id":2}, "signed":{"type":"text","value":signed,"group_id":2}, "witnessed":{"type":"text","value":witnessed,"group_id":2}}}

	if signed:
		eLabFTW_metadata["extra_fields"]["signed_at"] = {"type":"text", "value":signed_at, "group_id":1}
		eLabFTW_metadata["extra_fields"]["signed_by"] = {"type":"text", "value":signed_by, "group_id":1}
	if witnessed:
		eLabFTW_metadata["extra_fields"]["witnessed_at"] = {"type":"text", "value":witnessed_at, "group_id":1}
		eLabFTW_metadata["extra_fields"]["witnessed_by"] = {"type":"text", "value":witnessed_by, "group_id":1}
	if start_date != "null":
		eLabFTW_metadata["extra_fields"]["start_date"] = {"type":"text", "value":start_date, "group_id":1}
	if created_at != "null":
	...
	return 0, eLabFTW_project_data, alert_to_check, eLabFTW_links, attachments_filenames_for_project, uuid, links
	
if __name__ == "__main__":

	projects_file = "../ressourcen/experiments_downloaded.tsv"
	projects_downloaded = []
	if not os.path.isfile(projects_file):
		with open(projects_file, "w") as f:
			f.write("LabguruID\talert_to_check\tfile_name\teLabFTW_links\tattachments_list\tUUID\tlinked_uuids\n")
	else:
	...
	with open("Labguru_experiment_IDs.tsv") as f:
		header = True
		for line in f:
			if header:
				header = False
	...
	for project_number in list_of_project_numbers:
		print("Now working on experiment " + str(project_number))
		...
		if project_number in projects_downloaded:
			...
			return_code, eLab_json, alertCheck, eLabFTWLinks, attachments_list, uuid_id, link_uuids = download_experiment(project_number)
			if return_code:
				print("Experiment number " + str(project_number) + " seems to have failed")	
			else:
				print("Experiment number " + str(project_number) + " seems to have been a success")
				json_file_name = str(project_number) + ".json"
				with open("../ressourcen/experiment_jsons/" + json_file_name, "w") as jsonf:
					jsonf.write(str(eLab_json))
				with open(projects_file, "a") as f:
					f.write(str(project_number) + "\t" + str(alertCheck) + "\t" + json_file_name + "\t" + str(eLabFTWLinks) + "\t" + str(attachments_list) + "\t" + str(uuid_id) + "\t" + str(link_uuids) + "\n")
