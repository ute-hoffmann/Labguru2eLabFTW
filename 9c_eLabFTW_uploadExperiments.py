...
def remove_stuff(string):
	cleared_string = string.replace("</p>", "").replace("<p>","").replace("<br>","").replace("</p>","")
...
if __name__ == "__main__":
	uploaded_file = "../ressourcen/eLabFTW_experiments_uploaded.tsv"
	projects_uploaded = {}
	...
	with open(projects_file) as f:
		...
		for line in f:
			...
			json_name = "../ressourcen/experiment_jsons/" + split_line[2]
			if split_line[0] in ["1022268", "1019131", "1020251"]:
			...
			with open("LabguruID_eLabFTWID_all.tsv", "a") as f:
				f.write("experiments\t" + split_line[0] + "\t" + str(item_id) + "\n")
			...
			for attachment in attachments:
				...
				try:
					...
					with open("file_upload_problem.txt", "a") as f:
						f.write(str(split_line[0]) + "\t" + str(item_id) + "\tProblems with files, specifically:\t" + attachment + "\n")
