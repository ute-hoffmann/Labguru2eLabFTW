...
def download_elements_experiment_procedures(element_id):
	...
	if element_type == "text":
		...
		write_text += ""
		# I think everything also occurs as attachment anyways
		# better linking here !
		#for attachment in json.loads(element_json["data"].replace('\\"','')):
		#	attachment_id = attachment["id"]
		#	attach_info_download = requests.get(f'{BASE_URL}/attachments/{attachment_id}', params=standard_params, headers=standard_headers)
		#	if not attach_info_download.status_code == 200:
		#		raise ValueError("attachment id: " + str(attachment_id))
		#	ret_code, file_attachment = download_attachment(json.loads(attach_info_download.text))
		#	attachments_list_of_files.append(file_attachment)
		#	write_text += "<h2>Attachment: see below</h2>"
	elif element_type == "steps":
		string = element_json["data"].replace("\\n","").replace("\\t","").replace("<p>","").replace("</p>","").replace('<p style=\\\"\\\">','').replace('\\"','')
		string = json.loads(string.replace("\\",""))
		write_text += "<h2>Steps</h2>"
		for step in string:
			if step["title"]:
				write_text += "<p><strong>Next step:</strong>&emsp;" + step["title"] + " &emsp;"
			if step["timer"]["hours"] or step["timer"]["minutes"] or step["timer"]["seconds"]:
				write_text += "<strong>time (hh:mm:ss):</strong> " + step["timer"]["hours"] + ":" + step["timer"]["minutes"] + ":" + step["timer"]["seconds"]
				write_text += "</p>"
...
def download_project(project_number):
	r = requests.get(f"{BASE_URL}/projects/{project_number}", params=standard_params, headers=standard_headers)
	...
	# description = project_info["description"].replace('\t', ' ').replace('\n', ' ') # this also pops up as element in experimental_procedure -> get it from there!
	...
	#selected_teams = []
	#for selected_team in project_info["selected_teams"]:
	...
	for ID in eLabFTW_prel_links:
		...
		print(lab_ID)
		try:
			eLabFTW_links.append(Labguru_eLabFTW_dict[collection][lab_ID])
		except KeyError:
...
if __name__ == "__main__":
	...
	with open("Labguru_project_IDs.tsv") as f:
		header = True
		for line in f:
			if header:
				header = False
	...
	for project_number in list_of_project_numbers:
		print("Now working on project " + str(project_number))
		counter += 1
		#if counter == 10:
		#	break
		if project_number in projects_downloaded:
			...
			return_code, eLab_json, alertCheck, eLabFTWLinks, attachments_list, comments_list = download_project(project_number)
			...
			if return_code:
				...
				with open("../ressourcen/project_jsons/" + json_file_name, "w") as jsonf:
